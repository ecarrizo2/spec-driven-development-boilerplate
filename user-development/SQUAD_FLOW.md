...
| Epic definition | `epics/N-name/epic.md` | Jira Epic ticket |
| Task dependency graph | `epics/N-name/task-graph.md` | — |
| Individual requests | `epics/N-name/requests/` | Jira task tickets |
| Plans | Within the feature branch | PR diff |
| Delivery tracking | `epics/N-name/delivery.yaml` | — |
...
> **Acceptance criteria notation:** When this document includes success criteria or acceptance criteria, use EARS notation for system and API-level requirements. See [`common-specs/writing-specs.md`](../common-specs/writing-specs.md) for the five EARS patterns and Marketplace-domain examples.
...
2. Agent reads the completed `epic.md` and produces:
   ...
   - Request shells in `epics/N-name/requests/`
...
> **Acceptance criteria format:** During Phase 3 of the refinement session (Prompt 7), use EARS notation for system/API-level criteria and Given/When/Then for UI/behavioral flows. See [`common-specs/writing-specs.md`](../common-specs/writing-specs.md) for the complete pattern reference.
...
| **Output** | Implemented code + updated docs + plan `manifest.yaml` updated to `status: done` |
...
4. After all stages complete, agent:
   ...
   - Sets plan `manifest.yaml` → `status: done` (the plan folder stays in place)
...
4. If all epic tasks are done, TL runs `bin/dev wf:archive <epic-name>` to set the epic status to `done` in-place. The epic directory remains in `epics/`.
...
The model differs depending on whether the target repo has its own SDD.

#### Own-SDD repos (`has_own_sdd: true`)

Single branch, single PR. Plan, approval, and code all live together:

```mermaid
graph LR
    M[default branch]:::accent0 --> B[feature branch]:::accent1
    B --> P[plan committed]:::accent2
    P --> A[approval committed]:::accent3
    A --> E[execution commits]:::accent4
    E --> PR[PR ready for review]:::accent5
    PR --> MR[merged]:::accent0
```

Reviewers see what was planned alongside what was built. One PR, one merge.

#### Fallback-SDD repos (`has_own_sdd: false`)

Two branches, two PRs — one in the hub, one in the target repo:

```mermaid
graph TD
    HB[Hub: plan/GPMP-123_task branch]:::accent1
    HP[Hub draft PR → main]:::accent1
    RB[Repo: feat/GPMP-123_task branch]:::accent2
    RP[Repo PR → default branch]:::accent2

    HB -->|plan approved| RB
    RB -->|execution done, sync| HP
    HP -->|after repo PR merged| HM[hub PR merged to main]:::accent0
    RP -->|code review| RM[repo PR merged]:::accent2
```

| | Hub plan PR | Target repo PR |
|---|---|---|
| **Branch** | `plan/GPMP-123_task` | `feat/GPMP-123_task` |
| **Contains** | Plan files, task-graph/delivery updates, arch docs | Code + tests only |
| **Draft → Ready** | After execution done and target repo PR exists | After all stages pass |
| **Merged** | After target repo PR is merged | After code review passes |
...
Per `config/teams.yaml` — note the **underscore** separator between ticket ID and description:
```
<type>/<ticket-id>_<short-description>
```

Target repo examples:
- `feat/GPMP-123_add-review-snippets`
- `fix/GPMP-456_null-check-storefront-rating`
- `chore/GPMP-789_update-dependencies`

Hub-only branch types:
- `epic/GPMP-55110_review-snippets-on-storefronts`
- `plan/GPMP-123_add-review-snippets`

---

## Expected PR Contents

Understanding what a PR *should* contain helps reviewers focus and catch anomalies. Contents differ by SDD model.

### Hub Plan PR (Fallback-SDD repos — Steps 5-7)

This PR lives in `tkww/marketplace-sdd-hub` on a `plan/` branch:

| File type | Location | What to look for |
|-----------|----------|----------------|
| `manifest.yaml` | `fallback-sdd/<repo>/agent-development/plans/<task>/` | Correct status, complexity, stage breakdown |
| `specification.md` | Same folder | Clear overview, no `PENDING` markers, realistic pre-conditions |
| Stage files | Same folder | Logical ordering, verification steps, rollback plans |
| `task-graph.md` | `epics/<epic>/` | Task status updated |
| `delivery.yaml` | `epics/<epic>/` | PR URL + node status recorded |
| Architecture docs | `documentation/<repo>/` | Updated if plan introduces structural changes |

**Draft while:** plan is under review / execution is in progress  
**Ready for review when:** execution is complete and target repo PR exists  
**Links to:** target repo PR in description

### Plan PR (Own-SDD repos — Steps 5-6)

When first opened as a draft on the feature branch (same branch as execution):

| File type | Location | What to look for |
|-----------|----------|----------------|
| `manifest.yaml` | `sdd/agent-development/plans/<task>/` | Correct status, reasonable complexity, stage breakdown |
| `specification.md` | Same folder | Clear overview, all open questions resolved, realistic pre-conditions |
| Stage files | Same folder | Logical ordering, clear instructions, verification steps, rollback plans |
| Request file (unchanged) | `sdd/agent-development/requests/` | Should NOT be modified during planning |
...
For own-SDD repos, after execution the same branch/PR additionally contains:
...
| Spec updates | `sdd/agent-development/agent-specs/` | Updated if task introduced new patterns |
| Architecture docs | `architecture-documentation/` | Updated if structural changes were made |
| Updated `delivery.yaml` | `epics/<epic>/` (epic tasks only) | Node status → `ready-for-review` |

For fallback-SDD repos, the target repo PR contains **only** source code, tests, and documentation — no SDD artifacts.

**What should NOT be in any execution PR:**
...
3. Request goes into `agent-development/requests/` (standalone) or is added to the epic via TL
...
- [ ] Plan `status` in `manifest.yaml` set to `done`
...
- [ ] Epic `status` field in `epics/<epic>/epic.md` is set to `done`
...
1. TL or ENG creates request via Prompt 3 → `agent-development/requests/`
