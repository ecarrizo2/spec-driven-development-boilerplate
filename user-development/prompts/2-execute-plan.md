...
> the `sdd-execute-plan` skill activates automatically. In VS Code, use `/sdd-execute-plan`.
>
...
When `gh pr create --draft` runs it targets the **submodule's remote** (e.g., `tkww/TKMarketplace`), not the hub. The hub plan PR (`plan/` branch) is updated separately after execution as part of the Sync Rule.
...
8. If epic task: update `delivery.yaml` node status to `ready-for-review`
9. If epic task: update `task-graph.md` task status to `done`
