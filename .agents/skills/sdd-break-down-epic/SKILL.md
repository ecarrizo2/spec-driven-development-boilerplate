...
description: Decompose an approved cross-repo epic into a task graph, delivery manifest, and skeleton request shells — establishes the dependency DAG with one PR node per task.
---

## Identify the Epic

Before starting, determine which epic to break down:

1. **If you mentioned a specific epic folder** (e.g., "break down `epics/3-storefront-redesign/`") — use that path.
2. **If you mentioned an epic name or number** — find the matching folder in `epics/`.
3. **If unspecified** — list `epics/` and ask which epic to decompose.

Confirm the epic folder and verify `epic.md` has `status: pending` before proceeding.
