---
name: sdd-retro-analysis
description: Run a retrospective analysis on completed epic metrics — reads cycle data from metrics/<team-name>/cycles/ and produces structured talking points, calibration insights, and process improvement recommendations.
---

## Identify the Metrics Scope

Before starting, determine which metrics to analyze:

1. **If you mentioned a specific epic** — look for `metrics/<team-name>/cycles/<epic-name>.yaml`.
2. **If you said "all epics" or didn't specify** — read all files in `metrics/<team-name>/cycles/` (ignore `.gitkeep`).
3. **If no metrics files exist** — stop and report: "No metrics found — run `bin/dev audit:delivery <epic-id> --save` on completed epics first."

State which cycle files you'll analyze before proceeding.

---

## Context to Read

1. **All cycle files** in `metrics/<team-name>/cycles/` (or the one specified)
2. **The metrics schema** — `metrics/<team-name>/schema.yaml` (understand what each field means)
...
- "Tasks in `<repo-name>` consistently spawn amendments — is the architecture overview outdated, or is the codebase inherently more unpredictable?"
