---
name: sdd-amend-epic
description: Interactively amend an active in-flight epic — add new tasks, split existing ones, remove dropped requirements, or resequence dependencies while maintaining the negotiation audit trail.
---
...
## Identify the Epic and Amendment

Before starting, determine what to amend:

1. **If you described what changed** (new requirement, design revision, task too large) — confirm which epic is affected and describe the trigger.
2. **If you mentioned an epic and a change** — find the epic folder and verify its current state.
3. **If unspecified** — run `bin/dev status` to list active epics and ask which one to amend and what triggered the amendment.

State the epic and the amendment trigger before proceeding.
...
2. **The task graph** — `epics/<N-epic-name>/task-graph.md` (frontmatter for statuses + diagram for dependencies)
3. **The delivery manifest** — `epics/<N-epic-name>/delivery.yaml` (current PR state)
4. **All request files** — `epics/<N-epic-name>/requests/*.md` (to understand scope of each task)
...
9. **Writing specs** — `common-specs/writing-specs.md` for EARS notation when creating new request file shells in Phase 4.
...
4. **Create new request files** (for inserted tasks):
   - File: `epics/<N-epic-name>/requests/<ID>-short-name.md`
   - Use the shell format initially (status: `draft`) OR full refined format if enough information exists
   - Follow `agent-development/requests/_TEMPLATE-request.md` structure
...
6. **Update the epic status** — set to `paused` at the start of amendment, return to `active` after changes are applied
